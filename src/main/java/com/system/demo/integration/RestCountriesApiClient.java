...
public class RestCountriesApiClient {
    ...
    public Flux<Country> fetchAllCountries() {
        ...
        return webClient.get()
                ...
                .filter(response -> {
                    boolean passes = response.getPopulation() != null && response.getPopulation() > MIN_POPULATION;
                    if (!passes) {
                        log.debug("Filtered out country with population: {}", response.getPopulation());
                    }
                    return passes;
                })
    ...
    public Flux<Country> fetchCountriesByRegion(String region) {
        log.info("Fetching countries by region: {}", region);
        return webClient.get()
                .uri("/region/{region}", region)
                .retrieve()
                .bodyToFlux(RestCountriesCountryResponse.class)
                .filter(response -> response.getPopulation() != null && response.getPopulation() > MIN_POPULATION)
                .map(this::mapToCountry)
                .doOnError(error -> log.error("Error fetching countries by region: {}", region, error));
    }
    ...
    private Country mapToCountry(RestCountriesCountryResponse response) {
        ...
        country.setCapital(response.getCapital() != null && !response.getCapital().isEmpty()
                ? response.getCapital().get(0)
                : null);
        ...
        if (response.getCurrencies() != null && !response.getCurrencies().isEmpty()) {
            Map.Entry<String, RestCountriesCurrencyInfo> firstCurrency = response.getCurrencies().entrySet().iterator()
                    .next();
